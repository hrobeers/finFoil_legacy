...
!
  ...
  real    ( kind = 8 ), intent(out):: yval(dim_num)
!
!  Check.
!
...
subroutine spline_val ( dim_num, ndata, nval, tdata, ydata, tval, yval )
!*****************************************************************************80
!
!! SPLINE_VAL evaluates a speciefied spline in a range of values.
!
!  Discussion:
!
...
!    NDATA must be at least 3.
!
!    Input, integer ( kind = 4 ) NVAL, the number of abscissa values at which the spline
!    is to be evaluated.
!    NVAL must be at least 1.
!
!    Input, real ( kind = 8 ) TDATA(NDATA), the abscissas of the data
!    points.  The values in TDATA must be in strictly ascending order.
!
...
!    corresponding to the abscissas.
!
!    Input, real ( kind = 8 ) TVAL(NVAL), the abscissa value at which the spline
!    is to be evaluated.  Normally, TDATA(1) <= TVAL <= T(NDATA), and
!    the data will be interpolated.  For TVAL outside this range,
!    extrapolation will be used.
!
!    Output, real ( kind = 8 ) YVAL(DIM_NUM,NVAL), the value of the spline at TVAL.
!

!
! arguments
!
  integer(kind=4), intent(in):: ndata, dim_num, nval
  real(kind=8), intent(in):: tval(nval), tdata(ndata), ydata(dim_num,ndata)
  real(kind=8), intent(out):: yval(dim_num,nval)
...
!   integer(kind=4):: 
  ...
  do i = 1, nval
    call spline_overhauser_val(dim_num, ndata, tdata, ydata, tval(i), yval(:,i)) ! dim_num, ndata, tdata, ydata, tval, yval
  end do

end


subroutine bezier_curve_val ( n, tval, nval, con, bcval )
!*****************************************************************************80
!
!! BEZIER_CURVE_VAL evaluates a speciefied bezier spline in a range of values.
!
!  Discussion:
!
!    The other subroutines only evaluate in one point, this
...
!    N must be at least 0.
!
!    Input, real ( kind = 8 ) TVAL(NVAL), the points at which the Bezier curve should
!    be evaluated.  The best results are obtained within the interval
!    [0,1] but TVAL(NVAL) may be anywhere.
!
!    Input, integer ( kind = 4 ) NVAL, the number of abscissa values at which the spline
!    is to be evaluated.
!    NVAL must be at least 1.
!
!    Input, real ( kind = 8 ) CON(2,N+1), the control points of the bezier curve.
!	 The Bezier curve will pass through the points CON(:,0) and CON(:,N+1),
!    but generally NOT through the other control points.
!
!    Output, real ( kind = 8 ) bc_val(2,NVAL), the values of the bezier curve corresponding to TVAL.
!

!
! arguments
!
  integer(kind=4), intent(in):: n, nval
  real(kind=8), intent(in):: tval(nval), con(0:1,0:n)
  real(kind=8), intent(out):: bcval(2,nval)
...
!   integer(kind=4):: i
  ...
  do i = 1, nval
    call bc_val(n, tval(i), con(0,:), con(1,:), bcval(1,i), bcval(2,i)) ! n, t, xcon, ycon, xval, yval
  end do
